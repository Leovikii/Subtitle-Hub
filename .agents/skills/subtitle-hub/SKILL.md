...
0. For an existing project, require project schema 9, review schema 3, and `initialization.skill_version: "1.4.0"`. If any differ, upgrade that project to the current templates before audit, editing, build, validation, or release; never execute an older contract in place. Do not upgrade dormant projects proactively.
...
- For user-declared SSH video, use only the read-only Debian flow in the routed references. Never accept a password in chat or arguments, install remote tools, upload files, write remote outputs, or broaden the approved media paths.
