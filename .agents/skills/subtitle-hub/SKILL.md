...
0. For an existing project, require project schema 9, review schema 3, and `initialization.skill_version: "1.3.2"`. If any differ, upgrade that project to the current templates before audit, editing, build, validation, or release; never execute an older contract in place. Do not upgrade dormant projects proactively.
