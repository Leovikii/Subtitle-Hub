...
The builder asserts the master–candidate invariant immediately after construction; the independent release/package validator checks the final format. Masters permit no rendered font difference in the candidate. A noncurrent project must complete `SH-INIT-010` before this workflow; the candidate builder never upgrades schemas or fonts.
...
The deterministic package is `packages/bgm<subject-id> - <name_cn> [v<version>].zip` and contains current ASS files, VERSION, and CHECKSUMS only. `catalog.yaml` and `CATALOG.md` are also Action-owned derived outputs: do not edit, generate, or validate them locally. After a release push, the packaging Action builds packages. It regenerates both indexes only when `packages/` changed or the indexed package path, Raw GitHub URL, or jsDelivr CDN URL does not exactly match the ZIP set, then commits the package/index set together. `CATALOG.md` separates project metadata/report links under `项目资料` from the finished ZIP under `字幕成品`, with one-click original and CDN downloads. Package-link recovery ignores unrelated catalog fields, so ordinary metadata or report changes do not trigger index rebuilding. Project metadata and `review.md` remain their inputs and durable truths.
