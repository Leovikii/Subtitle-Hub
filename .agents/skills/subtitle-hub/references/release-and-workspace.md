...
The deterministic package is `packages/bgm<subject-id> - <name_cn> [v<version>].zip` and contains current ASS files, VERSION, and CHECKSUMS only. Use `.github/scripts/build_subtitle_packages.py`; after project or release changes run `scripts/sync_catalog.py` and `--check`. The catalog includes every project and links directly to metadata, current review, and available release/package.
