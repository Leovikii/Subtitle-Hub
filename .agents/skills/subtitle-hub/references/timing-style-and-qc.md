...
The remote command uses existing Debian tools through the shared Skill environment and the dedicated key enrolled once during SSH initialization. Media actions never request a password. They use one connection attempt, concurrency one, one decode thread, and remote/client timeouts; resolve the exact approved file; and reject symlink or path changes before reading. All outputs travel over the SSH channel; outside exact key enrollment or revocation, no Skill command may create files, caches, logs, directories, packages, containers, services, or configuration on the NAS. If key authentication or a required command fails, record the limitation and stop media escalation rather than prompting again, installing remote tools, or downloading the video.
