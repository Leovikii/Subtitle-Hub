...
The remote command uses existing Debian tools through the isolated Paramiko 5.0.0 backend, a user-verified pinned ED25519 key, one local graphical password prompt, one connection attempt, concurrency one, one decode thread, and remote/client timeouts. The password exists only in the current Python process. It resolves the exact approved file and rejects symlink or path changes before reading. All remote media outputs travel over the SSH channel; no Skill command may create files, caches, logs, directories, packages, containers, services, or configuration on the NAS. The server filesystem may update access time under its own mount policy; the Skill never changes that policy. If the connection or required command fails, record the limitation and stop media escalation rather than storing credentials, installing remote tools, or downloading the video.
