...
新建项目必须先主动探测音轨与字幕轨，并在一次确认中批准关联作品/范围、素材映射、发布语言和简短项目名。批准后 dry-run 作为内部安全检查；结果无实质变化时不得再打断用户。建项后以 `--ready-for-proofreading` 为内容门禁。
...
- 实质字幕修改必须先在 `docs/review.md` 提交详细分类校对方案，并写入 `docs/ledger.tsv`；重译、删减、增译和对白改正逐条列出，只有同规则同类别的机械修改可合并。方案批准后连续完成实现、验证和发行候选，不为常规检查另设中断；用户通过发行前终审后方可发布。
